...
MOVIEDB_REQUEST_FORMAT = "https://api.themoviedb.org/3/find/{}}?api_key=" + \
    MOVIEDB_API_KEY + "&language=en-US&external_source=imdb_id"
...
def update_eztv_data(num_pages=-1):
    ...
    while(run):
        ...
        response = requests.get(
            "{}/get-torrents?limit=50&page={}".format(EZTV_API, page))
        if response.status_code == 200:
            ...
            for show in shows:
                ...
                curr_show = shows_collection.find_one(
                    {"$or": [{'hash': show['hash']}, {'title': show_title}]})
                if not curr_show:
...
def scan_shows():
    ...
    for show in shows:
        ...
        result = re.match(r"(^[\w\s-]+)(S([0-9])*E([0-9]*))",
...
def enqueue_transmission():
    ...
    for candidate in download_candidates:
        download_dir = BASE_DOWNLOAD_PATH + "/{}/Season {}/{} S{}E{}.mkv".format(candidate['show_title'], candidate['season'], candidate['show_title'], candidate['season'], candidate['episode')
        print download_dir
